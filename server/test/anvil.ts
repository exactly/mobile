...
export default async function setup({ provide }: TestProject) {
  ...
  if (initialize) {
    ...
    shell.env.BROADCAST_WEBAUTHNOWNERPLUGIN_ADDRESS = await broadcast("WebauthnOwnerPlugin", "Plugin");
  ...
  const [issuerChecker, proposalManager, refunder, exaPreviewer, exaPlugin] = await Promise.all([
    broadcast("IssuerChecker"),
    broadcast("ProposalManager"),
    broadcast("Refunder"),
    broadcast("ExaPreviewer"),
    broadcast("ExaPlugin"),
  ]);
  const [, exaAccountFactory] = parse(
    ...
    await import(`@exactly/plugin/broadcast/ExaAccountFactory.s.sol/${foundry.id}/run-latest.json`),
  ).transactions;
...
async function broadcast(name: string, script = name) {
  return parse(
    object({ transactions: tuple([object({ contractName: literal(name), contractAddress: Address })]) }),
    await import(`@exactly/plugin/broadcast/${script}.s.sol/${foundry.id}/run-latest.json`),
