...
describe("proposal", () => {
  ...
  describe("with lost proposals", () => {
    ...
    it("execute proposal", async () => {
      const idleProposal = proposals[1]!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
      const withdraw = proposals[3]!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
      const anotherProposal = proposals[4]!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
            ...
            data: {
              ...
              block: {
                ...
                logs: [
                  ...
                  {
                    topics: anotherProposal.topics,
                    data: anotherProposal.data,
                    account: { address: anotherProposal.address },
                  },
                ],
              },
            },
      ...
      await vi.waitUntil(() => waitForTransactionReceipt.mock.settledResults.length >= 5);
      ...
      expect(
        idleProposalReceipt && idleProposalReceipt.type === "fulfilled"
          ? usdcToAddress(
              ...
              decodeAbiParameters([{ name: "receiver", type: "address" }], idleProposal.args.data)[0],
            )
          : 0n,
      ).toBe(idleProposal.args.amount);
    });
  });
});
