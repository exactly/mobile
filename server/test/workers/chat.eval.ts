...
async function evaluate() {
  ...
  for (const { name, scope, input, associated = false, called, contains = [], link, silent = [], rubric } of running) {
    ...
    const broken = await (async () => {
      ...
      if (link !== "/whatsapp") {
        return query === `${link.includes("?") ? "&" : "?"}${utm}` ? undefined : `expected an app link, got ${query}`;
      }
      if (!query.startsWith("?token=") || !query.endsWith(`&${utm}`)) return `no token on ${url}`;
      const subject = await whatsapp.decode(query.slice("?token=".length, -`&${utm}`.length)).catch(String);
