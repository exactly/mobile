...
describe("registration", () => {
  ...
  it.each([
    {
      name: "malformed",
      storedChallenge: JSON.stringify({ accountType: "business" }),
      expected: { code: "bad registration", legacy: "bad registration" },
    },
    {
      name: "mismatched",
      storedChallenge: encode("test-challenge", "business"),
      expected: { code: "bad account type" },
    },
  ])("rejects $name structured registration challenge", async ({ storedChallenge, expected }) => {
    await redis.set("test-session", storedChallenge);
    const response = await postRegistrationWebauthn();
