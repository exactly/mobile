...
-----END PUBLIC KEY-----`;
    ...
    describe("business application", () => {
      ...
      it("returns an existing company application without a status", async () => {
        ...
        const getCompanyStatus = vi.spyOn(panda, "getCompanyStatus").mockResolvedValue(companyApplication);
      ...
      it("returns a business application error", async () => {
        const error = new Panda.BusinessApplicationError(
          "business account is not complete",
          "processing",
          "kyb not approved",
        );
