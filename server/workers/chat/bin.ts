...
supervise(
  ...
  Promise.all([
    ...
    Promise.all([secret("chat-persona-api-key", secrets), secret("persona-api-url", secrets)]).then(([key, url]) =>
  ...
  ]).then(([anthropicKey, whatsapp, persona, database, [bullmq, store]]) =>
    own(
      worker({ anthropicKey, bullmq, database, persona, store, whatsapp }),
      () => bullmq.quit(),
      () => database.$client.end(),
      () => secrets.close(),
      () => store.close(),
