...
read -ra input <<< "$(cast abi-decode "_()(uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256)" "$1" | sed 's/ .*//' | xargs)"
...
rate=$(BC_LINE_LENGTH=666 bc -l <<< "
  ...
  maxrate     = ${input[6]} / wad
  linearr     = ${input[7]} / wad
  amountstime = ${input[8]} / wad
  total       = ${input[9]} / wad

  avgduration = amountstime / total

  if (avgduration <= dstart) {
    rate = minrate
  } else if (avgduration >= dend) {
    rate = maxrate
  } else {
