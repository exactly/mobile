...
contract ExaPlugin is AccessControl, BasePlugin, IExaAccount {
  ...
  function collectCollateral(
    ...
    _depositRemainder(collateral, maxAmountIn - amountIn, msg.sender);
    _depositApprovedRemainder(EXA_USDC, amountOut - amount, msg.sender);
  ...
  function receiveFlashLoan(IERC20[] calldata, uint256[] calldata, uint256[] calldata, bytes calldata data) external {
    ...
    _depositApprovedRemainder(EXA_USDC, amountOut - actualRepay, c.borrower);

    _depositRemainder(c.marketIn, c.maxAmountIn - amountIn, c.borrower);
  ...
  function _depositApprovedRemainder(IMarket market, uint256 remainder, address receiver) internal {
    if (remainder != 0) market.deposit(remainder, receiver);
  }

  function _depositRemainder(IMarket market, uint256 remainder, address receiver) internal {
    if (remainder != 0) {
      IERC20(market.asset()).approve(address(market), remainder);
      market.deposit(remainder, receiver);
    }
  }
