...
contract ExaPlugin is AccessControl, BasePlugin, IExaAccount {
  ...
  {
    if (functionId == uint8(FunctionId.PRE_EXEC_VALIDATION_AUDITOR_CALLED)) {
  ...
  function postExecutionHook(uint8 functionId, bytes calldata preExecHookData) external override {
    ...
    if (functionId == uint8(FunctionId.PRE_EXEC_VALIDATION_AUDITOR_CALLED)) return;
    revert NotImplemented(msg.sig, functionId);
  }
  ...
  function pluginManifest() external pure override returns (PluginManifest memory manifest) {
    ...
    ManifestFunction memory exitMarketValidationFunction = ManifestFunction({
      functionType: ManifestAssociatedFunctionType.SELF,
      functionId: uint8(FunctionId.PRE_EXEC_VALIDATION_AUDITOR_CALLED),
      dependencyIndex: 0
    });
...
enum FunctionId {
  ...
  PRE_EXEC_VALIDATION_AUDITOR_CALLED
}
