...
contract ExaPlugin is AccessControl, BasePlugin, IExaAccount {
  ...
  function pluginManifest() external pure override returns (PluginManifest memory manifest) {
    ...
    manifest.executionFunctions[5] = this.collectCollateral.selector;
    manifest.executionFunctions[6] = bytes4(keccak256("collectCredit(uint256,uint256,uint256,bytes)"));
    manifest.executionFunctions[7] = bytes4(keccak256("collectCredit(uint256,uint256,uint256,uint256,bytes)"));
    manifest.executionFunctions[8] = this.collectDebit.selector;
